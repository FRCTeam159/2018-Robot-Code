...
public class DrivePath extends Command implements PhysicalConstants, RobotMap {
    ...
    DrivePath() {
        ...
        if (!newgameMessage.equals(Robot.fmsData)) {
            System.out.println(
                    "WARNING DrivePath FMS different from autoInit now:" + newgameMessage + " was:" + Robot.fmsData);
        ...
        targetString = "Position:" + whichPosition[robotPosition] + " Target:" + whichSide[targetSide] + " - "
                + whichObject[targetObject];
    ...
    protected boolean isFinished() {
        ...
        if (pushing) {
            ...
            if (targetObject == SCALE) {
                ...
                if (Robot.elevator.getPosition() > Elevator.SCALE_HEIGHT - Elevator.MOVE_RATE
                        && Robot.elevator.getPosition() < Elevator.SCALE_HEIGHT + Elevator.MOVE_RATE) {
                    pushing = true;
                    pushTimer.reset();
                    pushTimer.start();
                }
    ...
    private Trajectory calculateTrajectory(String gameData, int robotPosition, double maxVelocity,
